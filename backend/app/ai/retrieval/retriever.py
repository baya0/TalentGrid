...
class Retriever:
    ...
    def retrieve(self, user_query: str, ui_filters: dict, top_k: int = 20):
        ...
        logger.info(f"[RETRIEVER] Query: '{clean_query}' | ChromaDB filters: {filters} | UI filters: {ui_filters}")
        ...
        has_sql_filters = bool(ui_filters and (
            ui_filters.get("languages") or ui_filters.get("location")
        ))
        ...
        # ----------------------------------
        if filters:
            logger.info(f"[RETRIEVER] Filtered search: fetching {fetch_k} chunks from ChromaDB")

            ranked, docs = self.hybrid.search(
                query_vector,
                filters=filters,
                k=fetch_k
            )
        ...
        if not ranked:
            ...
            logger.info(f"[RETRIEVER] No filtered results, fallback to unfiltered search (k={fallback_k})")
        ...
        # Calculate final score per CV: use MAX of chunk scores
        # This ensures score stays in 0-1 range
        for cv_id in results:
            chunk_scores = results[cv_id]["chunk_scores"]
            # Use max score - the best matching chunk represents the CV
            results[cv_id]["score"] = max(chunk_scores) if chunk_scores else 0
        ...
        logger.info(f"[RETRIEVER] Grouped into {len(candidates)} unique CVs from {len(ranked)} chunks")
        if candidates:
            top_scores = sorted([c["retrieval_score"] for c in candidates], reverse=True)[:5]
            logger.info(f"[RETRIEVER] Top retrieval scores: {[round(s, 3) for s in top_scores]}")
