...
class HybridSearch:
    ...
    def _is_skill_query(self, query):
        ...
        ONLY short queries (1-2 meaningful words) are treated as skill queries.
        senior ai developer" should NOT be a skill query — it needs semantic
        understanding of the full phrase together.
        """
        words = [w for w in query.lower().split() if w not in self.STOP_WORDS]
        # ONLY short queries (1-2 words) use keyword-heavy weights
        # e.g., "flutter", "react", "python developer"
        # NOT "senior ai developer" or "experienced machine learning engineer"
        return len(words) <= 2
        ...
        for i, word in enumerate(query_words):
            if word in self.TECH_SKILLS:
                ...
                if i + 1 < len(query_words) and query_words[i + 1] in title_keywords:
                    ...
                    if (
                        word in text_lower[:200]
                    ):  # Check in first 200 chars (title/summary area)
                        boost += 0.2
    ...
    def bm25_score(self, query, documents):
        ...
        # ── NEW: Keyword coverage penalty ──
        # If query has 3 keywords but a doc only matched 1, dampen the score.
        # Without this, the "best" doc always gets 1.0 even if it's a weak match.
        if len(keywords) > 1:
            for doc_id, text in documents.items():
                text_lower = text.lower()
                matched = sum(1 for w in keywords if w in text_lower)
                coverage = matched / len(keywords)
                # Apply coverage: matching 1/3 keywords → score * 0.33
                scores[doc_id] = scores.get(doc_id, 0) * max(coverage, 0.1)
    ...
    def search(self, query, filters=None, k=20):
        ...
        vector_results = self.store.search(
            query_embedding=query, n_results=k, where=filters
        )
        ...
        if is_skill:
            ...
            keyword_weight = 0.8
            logger.info(
                f"[HYBRID] Query type: SKILL → weights: {keyword_weight*100:.0f}% keyword, {semantic_weight*100:.0f}% semantic"
            )
        else:
            ...
            keyword_weight = 0.4
            logger.info(
                f"[HYBRID] Query type: JOB_DESC → weights: {semantic_weight*100:.0f}% semantic, {keyword_weight*100:.0f}% keyword"
            )
